...
from utils.helpers import format_currency, get_current_year_month, get_effective_status
...
def show_dashboard():
    ...
    with st.spinner("データを読み込み中..."):
        stats = get_dashboard_stats(year, month)
        df = get_payments_df(year, month)
    ...
    with c1:
        st.metric(
            "今月 支払件数",
            f"{stats['total_count']} 件",
            help="今月登録されている支払い件数"
        )
    with c2:
        st.metric(
            "今月 支払総額",
            format_currency(stats["total_amount"]),
            help="今月の支払い合計金額"
        )
    with c3:
        st.metric(
            "支払済み",
            f"{stats['paid_count']} 件",
    ...
    with c6:
        st.metric(
            "📅 今週 支払い予定",
            f"{stats['this_week_count']} 件",
            help="本日から7日以内に支払日が来る件数"
        )
    with c7:
        st.metric(
            "🔒 固定支払い",
            f"{stats['fixed_count']} 件",
            help="今月の固定支払い件数"
        )
    with c8:
        st.metric(
            "🔄 変動支払い",
            f"{stats['variable_count']} 件",
            help="今月の変動支払い件数"
        )
    ...
    with col_chart1:
        ...
        if stats["total_count"] > 0:
            ...
            unpaid = stats["unpaid_count"] - stats["overdue_count"]
            overdue = stats["overdue_count"]
            fig = go.Figure(data=[go.Pie(
                labels=["支払済み", "未払い", "期限超過"],
                values=[paid, max(0, unpaid), overdue],
                hole=0.45,
    ...
    if not df.empty:
        df["eff_status"] = df.apply(
            lambda r: get_effective_status(r["status"], r.get("adjusted_date", "")),
            axis=1
        )
        week_df = df[
        ...
        if not df.empty:
            ...
            if not overdue_df.empty:
                ...
                overdue_df["経過日数"] = overdue_df["adjusted_date"].apply(
                    lambda d: f"{(date.today() - date.fromisoformat(d)).days}日超過"
                    if d else ""
                )
